...
def load_hamer(package_path, checkpoint_path=DEFAULT_CHECKPOINT):
    from pathlib import Path
    from ..configs import get_config
    model_cfg = str(Path(checkpoint_path).parent.parent / 'model_config.yaml')
    model_cfg = get_config(package_path + model_cfg, update_cachedir=True)
    ...
    if ('PRETRAINED_WEIGHTS' in model_cfg.MODEL.BACKBONE):
        ...
        model_cfg.MANO.MEAN_PARAMS = package_path + model_cfg.MANO.MEAN_PARAMS[1:]
        model_cfg.MANO.DATA_DIR = package_path + model_cfg.MANO.DATA_DIR[1:]
        model_cfg.MANO.MODEL_PATH = package_path + model_cfg.MANO.MODEL_PATH[1:]
        model_cfg.freeze()
        
    model = HAMER.load_from_checkpoint(package_path + checkpoint_path, strict=False, cfg=model_cfg)
