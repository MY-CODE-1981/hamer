#!/home/initial/miniconda3/envs/hamer/bin/python
# -*- coding: utf-8 -*-
import cv2
...
from hamer_ros.msg import hand_info
...
class ObjectDetecton:
    def __init__(self):
        ...
        self.hand_info_msgs = hand_info()
        self.hand_info_pub = rospy.Publisher("/camera_head/hand_info", hand_info, queue_size=10)
    ...
    def callback(self, image_raw, camera_info):
        ...
        # cv2.waitKey(1)
        if cv2.imwrite("/home/initial/workspace/homer_ws/src/homer/third-party/ycb_render/output/head_hamer.jpg", img_cv2,  [cv2.IMWRITE_JPEG_QUALITY, 100]):
            print("画像の保存に成功しました。")
        else:
            print("画像の保存に失敗しました。")
        ...
        if len(bboxes) != 0:
            ...
            for batch in dataloader:
                ...
                print("##########################")
                print("model_cfg.EXTRA.FOCAL_LENGTH: ", model_cfg.EXTRA.FOCAL_LENGTH, scaled_focal_length)
                print("##########################")
                pred_cam_t_full = cam_crop_to_full(pred_cam, box_center, box_size, img_size, scaled_focal_length).detach().cpu().numpy()
    
                end_time = time.time(); print("time2: ", end_time - start_time, " [sec]"); 
                # Render the result
                ...
                for n in range(batch_size):
                    ...
                    # cam_t = out['pred_cam_t'][n].detach().cpu().numpy()# pred_cam_t_full[n]
                    
                    # データ保存
                    import os
                    file_name = "output/mesh/np_savez_color.npz"
                    ...
                    np.savez("/home/initial/workspace/homer_ws/src/homer/third-party/ycb_render/output/data.npz", hand_mesh=verts, translate=cam_t)

                    all_verts.append(verts)
                    all_cam_t.append(cam_t)
                    all_right.append(is_right)
                    
                    self.hand_info_msgs.header = image_raw.header
                    self.hand_info_msgs.hand_mesh = verts.reshape(-1).tolist()
                    self.hand_info_msgs.hand_skelton = keyp.reshape(-1).tolist()
                    self.hand_info_msgs.camera_trans = cam_t.tolist()
                    # self.hand_info_msgs.camera_trans = out['pred_cam_t'][n].detach().cpu().numpy().tolist() # cam_t.tolist()
                    
                    self.hand_info_pub.publish(self.hand_info_msgs)
...
if __name__ == '__main__':
    rospy.init_node("demo_d435i")
    objDet = ObjectDetecton()
    rospy.spin()